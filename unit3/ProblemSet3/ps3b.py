...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        # TODO
        flag = random.random()
        if flag <= self.maxBirthProb*(1-popDensity):
...
class Patient(object):
    ...
    def __init__(self, viruses, maxPop):
        ...
        """
        self.viruses = viruses#list of instances
        self.maxPop = maxPop
    ...
    def update(self):
        ...
        integer)
        """
        lst_virus = self.viruses[:]
        for v in lst_virus:
            if v.doesClear():
                self.viruses.remove(v)
        current_pop = self.getTotalPop()
        current_pop_dense = float(current_pop / self.maxPop)
        lst2_virus = self.viruses[:]
        for p in lst2_virus:
            try:  
                self.viruses.append(p.reproduce(current_pop_dense))
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if flag <= self.maxBirthProb * (1 - popDensity):
            ...
            for drug in self.resistances:
                flag_inherit = random.random()
                if flag_inherit <= self.mutProb:
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        if len(drugResist) == 0 or count == 0:
            return count
        else:
    ...
    def update(self):
        ...
        for p in lst2_virus:
            try:  
                self.viruses.append(p.reproduce(current_pop_dense, self.prescription))
            except NoChildException:
                continue
        return len(self.viruses)        
