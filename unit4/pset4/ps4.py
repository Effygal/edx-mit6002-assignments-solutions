...
def r_squared(y, estimated):
    ...
    r2 = 1 - np.sum((y-estimated)**2) / np.sum((y-np.mean(y))**2)
    return float(r2)
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        pylab.plot(xVals, yVals,"bo", label = "Actual values")
        pylab.title('Temperature vs. time')
        pylab.xlabel('Year')
        pylab.ylabel('Temperature')
        estimated = pylab.polyval(model, xVals)
        pylab.plot(xVals, estimated, label = 'R^2 =' + str(r_squared(yVals, estimated)))
